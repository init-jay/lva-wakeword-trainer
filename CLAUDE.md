...
- **Treat `deploy/` as staging ground.** It stages ONE candidate per satellite -
  ...
  ledger; `deploy/README.md` keeps the rules and the traps, not the numbers; `deploy/<word>.md`
  keeps the per-word narrative a row cannot carry - what is on each satellite, what was
  retracted, what no measurement has cleared), out of
