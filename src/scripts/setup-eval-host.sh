...
ENV_DIR="src/eval"
CLONE="openwakeword"
CLONE_DIR="$(pwd)/src/train/openwakeword"
...
if [[ ! -d "$CLONE_DIR/openwakeword" ]]; then
    echo "ERROR: no $CLONE_DIR/ clone - the .onnx path imports openwakeword.model." >&2
    echo "       ./src/scripts/setup-applesilicon-trainer.sh creates it (with the" >&2
    echo "       embedding models); the mWW and .tflite paths need no clone." >&2
    exit 2
fi
for m in embedding_model melspectrogram; do
    if [[ ! -f "$CLONE_DIR/openwakeword/resources/models/$m.onnx" ]]; then
        echo "ERROR: $CLONE_DIR/openwakeword/resources/models/$m.onnx is missing." >&2
        echo "       ./src/scripts/setup-applesilicon-trainer.sh fetches both; without" >&2
...
# The clone is a sibling of the trainers (src/train/), not of src/eval/, so the
# install target is $CLONE_DIR, not the pre-reorg ../$CLONE.
( cd "$ENV_DIR" && VIRTUAL_ENV="$ENV_DIR/.venv" uv pip install --no-deps -e "$CLONE_DIR" --quiet )
...
"$ENV_DIR/.venv/bin/python" "$ENV_DIR/src/eval_model.py" --help >/dev/null
"$ENV_DIR/.venv/bin/python" "$ENV_DIR/src/compare_models.py" --help >/dev/null
