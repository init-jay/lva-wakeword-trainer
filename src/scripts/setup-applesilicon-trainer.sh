...
cd "$(dirname "$0")/../../"
REPO_ROOT="$(pwd)"
ENV_DIR="src/train/train-applesilicon"
CLONE="openwakeword"
...
# project (src/tts-service/engines/kokoro_mlx) that reads it at its own startup and
...
if [[ ! -f "$ESPEAK_DATA/phontab" ]]; then
    ...
    echo "         The Kokoro engine (src/tts-service/engines/kokoro_mlx) will not start."
...
for p in skip-piper-import honour-augmentation-rounds feature-device-selection \
    ...
    python3 "src/train/patches/$p.py" "$CLONE_DIR/openwakeword/train.py"
done
...
MODELS="$CLONE_DIR/openwakeword/resources/models"
mkdir -p "$MODELS"
...
#   src/tts-service/engines/kokoro_mlx the Kokoro engine (MLX, protocol port 8900)
#   src/tts-service/engines/piper      the Piper engine (in-process piper-tts, 8898)
#
...
for proj in "$ENV_DIR" src/tts-service/engines/kokoro_mlx src/tts-service/engines/piper; do
    echo "==> syncing $proj"
...
# raw forms it used to misread as a different backend. A bad SPEC is a ValueError
# from _parse_spec (TtsProtocolError is for server-side failures), so the loop
# catches the former - catching the latter let the ValueError escape and abort
# setup (pre-existing at e5cb5f0; the bad-spec half of this check had never run).
...
# The protocol's timestamps are a LIST of word-time dicts (wire.py), not a
