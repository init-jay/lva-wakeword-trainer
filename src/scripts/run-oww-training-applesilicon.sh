...
# src/tts-service/engines/, each in its own venv -
#
#     uv run --project src/tts-service/engines/kokoro_mlx python -m kokoro_mlx_engine \
#         --port 8900        # Kokoro, in-process MLX, protocol port 8900
#     uv run --project src/tts-service/engines/piper python -m piper_engine \
...
# src/tts-service/engines/kokoro_mlx) on protocol port 8900. Export a default only
...
#
# On a Mac the Piper engine is in-process (src/tts-service/engines/piper: piper-tts
...
set +e
script -q "$LOG" env PYTHONPATH=src "$ENV_DIR/.venv/bin/python" -m train.oww.train \
