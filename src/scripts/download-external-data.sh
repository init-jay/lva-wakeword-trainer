...
#
#   ./src/scripts/download-external-data.sh            # all - the default
#   ./src/scripts/download-external-data.sh oww        # openWakeWord only
#   ./src/scripts/download-external-data.sh mww        # microWakeWord only
#
#   docker compose run --rm oww-trainer ./src/scripts/download-external-data.sh
#   docker compose run --rm mww-trainer ./src/scripts/download-external-data.sh mww
#
...
if [ "$want_mww" = 1 ]; then
    ...
    if [ -n "$missing" ]; then
        ...
        echo "               ./src/scripts/download-external-data.sh oww"
        echo
    fi
