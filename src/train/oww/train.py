...
# corpus under train/oww/ and trains on nothing. The pre-reorg parents[2] now
# points at src/ - the GIT root is one level up, because the code moved under
...
def report_onnx_providers():
    ...
    melspec = OWW_CLONE / "openwakeword" / "resources" / "models" / "melspectrogram.onnx"
...
def create_config(wake_word: str, n_samples: int, training_steps: int,
    ...
    default_path = OWW_CLONE / "examples" / "custom_model.yml"
...
def run_augmentation(overwrite: bool = False):
    ...
    train_script = str(OWW_CLONE / "openwakeword" / "train.py")
